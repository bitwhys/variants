...
export const styles = {
  base: [
    ...
    // Sizing
    'px-[calc(theme(spacing[3.5])-1px)] py-[calc(theme(spacing[2.5])-1px)] sm:px-[calc(theme(spacing.3)-1px)] sm:py-[calc(theme(spacing[1])-1px)] sm:text-sm/6',
    // Focus
...
export const Button = React.forwardRef(function Button(
  ...
  const classes = cx(
    ...
    outline
      ...
      : plain
        ? styles.plain
        : cx(styles.solid, styles.colors[color ?? 'dark/zinc']),
  )
  ...
  return 'href' in props ? (
    ...
    <Headless.Button {...props} className={cx(classes, 'cursor-default')} ref={ref}>
      <TouchTarget>{children}</TouchTarget>
    </Headless.Button>
  )
})
